...
def orderling_list(self):
    ...
    total = 0
    print(
        f"=====　Listas de pedidos por cliente({self.name}, {self.shop})　=====")
    for order in self.orders:
        print(
            f"Nombre del cliente: {order[0]}, cantidad de dinero: {order[1]}, fecha y hora：{order[2]}")
        total += order[1]
...
def _shopping_list(self, basket):
    ...
    for grocery in basket:
        ...
        print(
            f"nombre comercial (marca): {name}, volumen: {quantity}, cantidad de dinero：{money}")
        self.total += money
